...
const subjectUser = async (req, res) => {
    const {id} = req.params;
    const subjects = await pool.query('SELECT nom_mat FROM estudiante AS es,materia AS mat,detalle_materia AS dt WHERE mat.id_mat = dt.cod_mat AND dt.cod_es = es.id_es AND es.id_es = ?',[id]);
